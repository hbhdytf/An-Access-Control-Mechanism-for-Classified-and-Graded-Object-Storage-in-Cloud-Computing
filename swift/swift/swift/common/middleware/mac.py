    ...
    def __call__(self, env, start_response):
        ...
        if req.method == 'GET' and obj:
            ...
            if sub.secfield is None :
                start_response("403 Forbidden", [("Content-type", "text/plain"), ])
                return ["User %s does not have secfield\n" % user.username]
            for secfd in sub.secfield:
	...
	print "=======================method PUT:"
        if method == 'PUT':
            try:
                ...
                print "elf.cur.execute('INSERT INTO TMeta(object_name, parent_secl_id, obj_seclevel, author, gen_time, path, type, subject, description, language, source) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)', values)",values
            except Exception as e:
