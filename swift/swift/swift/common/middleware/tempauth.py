...
class TempAuth(object):
    ...
    def __init__(self, app, conf):
        ...
        self.conn = MySQLdb.connect(host="localhost", user="root", passwd="root", db="auth", charset="utf8")
        #cur = conn.cursor()
        cur1 = conn.cursor()
    ...
    def get_groups(self, env, token):
        ...
        """
        groups = None
        print('----in get_groups---',self.users)
	...
	try:
	    cur = self.conn.cursor()
            cur.execute('select * from TUser where login_name = "%s"' % account)
        except:
            self.conn = MySQLdb.connect(host="localhost", user="root", passwd="root", db="auth", charset="utf8")
            cur = self.conn.cursor()
            cur.execute('select * from TUser where login_name = "%s"' % account)
        for row in cur.fetchall():
            user = row[3].encode('utf-8')
            url = '$HOST/v1/%s%s' % (self.reseller_prefix, 'mac')
            self.users[user] = {'url': url}
        print('account---',account,self.users)
