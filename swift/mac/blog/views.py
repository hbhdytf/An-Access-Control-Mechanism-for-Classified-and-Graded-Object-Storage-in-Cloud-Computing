...
def delmByID(request, me_id, uname, id):
    ...
    try:
        auth_token = cache.get(name_token).split(':')[-1].strip()
...
def showmeta(request, meid, uname):
    ...
    for i in range(mt.count()):
        ...
        #if obj填写安全级别
        if int(objseclevel)==1:
            objseclevel = '公开'
        elif int(objseclevel) == 2:
            #objseclevel = '内部'
            objseclevel = '公开'
        elif int(objseclevel) == 3:
            objseclevel = '公开'
        elif int(objseclevel) ==4:
            objseclevel = '内部'
        elif int(objseclevel) ==5 :
            objseclevel = '机密'
